...
install_ssg_debian_packages() {
    ...
    if [ ! -d "${ssg_dir}" ]; then
        log "Creating ${ssg_dir} for SCAP content"
        mkdir -p "${ssg_dir}" 2>> "${LOG_FILE}" || {
            log "Cannot create ${ssg_dir} for SCAP content" "ERROR"
            return 1
        }
    else
        log "Using existing ${ssg_dir} for SCAP content"
    fi
